...
* Fetching complicated resources with nested hierarchies require multiple round trips between the client and server to render single views, such as fetching content of a blog entry and the comments on that entry. For mobile applications operating in variable network conditions, these multiple roundtrips are highly undesirable.
* Over time, more fields might be added to an API response and older clients will receive all new data fields, even those that they do not need, which bloats the payload size and leads to larger latencies.
