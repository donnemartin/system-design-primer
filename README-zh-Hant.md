...
> 常见面向对象设计面试问题及样例讨论，代码和图标演示。
>
> 与内容有关的解决方案在 `solutions/` 文件夹中。
...
首先，你应对一般性原则有着基本理解，知道它们是什么，怎样使用以及利弊。
...
如果服务**性能**的增长与资源的增加是成比例的，服务是可扩展的。通常，提高性能意味着服务于更多的工作单元，另一方面，当数据集增长时，同样也可以处理更大的工作单位。<sup><a href=http://www.allthingsdistributed.com/2006/03/a_word_on_scalability.html>1</a></sup>

另一种方式来看待性能与可扩展性(关系):

* 如果你的系统有 **性能** 问题，对于单个用户来说是缓慢的。
* 如果你的系统有 **可扩展性** 问题，单个用户较快但在高负荷下会慢。
...
*网络不可靠，所以你应要支持分区容错性，并需要在软件可用性和一致性间做出取舍。*

#### CP 方式 - 一致性和分区容错性

等待分区节点的响应可能会导致延时错误。如果你的业务需求需要原子读写，CP 方式是一个不错的选择。

#### AP 方式 - 可用性与分区容错性

响应返回的最近版本数据可能并不是最新的。当分区解析完后，写入(操作)可能要花一些时间来传播。

如果业务需求允许[最终一致性](#eventual-consistency)或当有外部故障时要求系统继续运行，AP 方式是一个不错的选择。
...
此种方式在 DNS 和 email 等系统中使用。最终一致性在高可用性系统中效果不错。

### 强一致性

在写入后，访问立即可见。数据被同步复制。

此种方式在文件系统和关系型数据库(RDBMS)中使用。强一致性在需要记录的系统中运作良好。
...
* DNS 服务最近遭受 [DDoS 攻击](http://dyn.com/blog/dyn-analysis-summary-of-friday-october-21-attack/)，防止人们访问 Twitter 等网站而不知道 Twitter 的 IP 地址。
...
当你服务器上内容发生变动时，推送 CDN 接受新的内容。你负责提供内容，直接接推送至 CDN 并重写 URL 地址以指向 CDN 地址。你可以配置内容到期时间及何时更新。内容只有在更改或新增是才推送，最小化流量，但最大化存储空间。
...
CDN 拉取是当第一个用户请求该资源时，从服务器上拉取资源。你将内容留在自己的服务器上并重写 URL 指向 CDN 地址。直到内容被缓存在 CDN 上，这样请求会更慢。

[存活时间(TTL)](https://en.wikipedia.org/wiki/Time_to_live)决定缓存多久时间。CDN 拉取方式小化 CDN 上的储存空间，但如果过期文件并在实际更改之前被拉取，则会导致冗余的流量。
...
* **Session 留存** - 如果 Web 应用程序不追踪回话，发出 cookie 并将特定客户端的请求路由到同一实例。

通常会设置采用[工作-备用](#active-passive) 或 [active-active](#active-active) 模式的多个负载均衡器，以免发生故障。
...
7 层负载均衡器根据监看[应用层](#communication)来决定怎样分发请求。这会涉及请求头的内容，消息和 cookie。7 层负载均衡器终结网络流量，读取消息，做出负载均衡判定，然后连接选定的特定服务器。比如，一个 7 层负载均衡器能直接将视频流量连接到托管视频的服务器，同时将更敏感的用户账单流量引导到安全性更强的服务器。

以损失灵活性为代价，4 层负载均衡比 7 层负载均衡只需更少时间和计算资源，虽然这在现代商用硬件上的性能影响甚微。

### 横向扩展

负载均衡器还能帮助横向扩展，提高性能和可用性。使用商业硬件的性价比更高，并且比在单台硬件上**纵向扩展**更贵的硬件具有更高的可用性。招聘商业硬件的人才比特定企业系统的人才更容易。
...
* 缓存和数据库等下游服务器需要随着上游服务器扩展，处理更多的并发连接。
